...
// Map
//funcao semelhante à operação Map, que recebe uma coleção e uma função transformadora
// mapeando todos os elementos e aplicando uma regra em cada um deles, retornando o
// novo vetor resultante da operação
export function meu_map(colecao, funcao_transformadora){
    const novo_vetor = []
    for (let elemento of colecao){
        meu_push(novo_vetor, funcao_transformadora(elemento))
    }
    return novo_vetor
}

// Filter
// funcao semelhante à operação Filter, que recebe uma coleção e uma regra ou predicado de valor booleano (true ou false) 
// para cada elemento do vetor, se o elemento atender ao predicado retornando true, o elemento é adicionado ao novo vetor
// realizando a operação de filtragem por uma regra
...
// Reduce
// funcao semelhante à operação Reduce, que recebe uma coleção, uma função e um valor inicial de acumulador
// Dado um vetor, extrai apenas uma informação a partir do processamento de todos os elementos juntos, seguindo uma regra qualquer
export function meu_reduce(colecao, funcao, valor_inicial){
    let acumulador = valor_inicial
    for (let elemento of colecao){
        acumulador = funcao(elemento, acumulador)
    }
    return acumulador
}
...
export function cria_vetor_numerico_arquivo(){
    try{
        ...
        return meu_map(vetor, (elemento) => parseInt(elemento))
    } 
    catch(error){
